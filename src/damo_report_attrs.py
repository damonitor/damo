...
def percentiles_str(
        snapshot, record, probe_weights, fmt, recency_or_temperature):
    if len(snapshot.regions) == 0:
        return 'no region in snapshot'
...
def adjust_region_sz(damo_records, adjust_rules):
    ...
    if adjust_rules is None:
        return None
    for damo_record in damo_records:
        kdamonds = damo_record.damon_records.kdamonds
        for damon_record in damo_record.damon_records.record_list:
